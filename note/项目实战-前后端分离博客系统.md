...
​	简单来说，就是使用VO来封装前端需要的数据，前端不需要的不封装
...
涉及分类的时候，一般会保持有有父id，这样能够设计分类。 最顶层的分类的父id为-1 。其余的就进行对应就好。进行自关联
...
对于展示在前端页面的时间的从Java类转换成为Json格式的默认，我们需要修改成其他样子的。
...
import com.chang.annotation.SystemLog;
...
public class LogAspect {
    ...
    // 切点,当我们使用这个注解的时候，代表使用了AOP进行了注解。
    @Pointcut("@annotation(com.chang.annotation.SystemLog)")
    public void pt(){

    }
    // 通知方法 使用这个切点的时候，进行方法的增强。
    @Around("pt()") // 环绕通知，功能最强大
    public Object pointLog(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            handleAfter(ret);
        } finally { // 需要finally进行最终的处理
            // 结束后换行 System.lineSeparator()系统的换行符
            log.info("=======End=======" + System.lineSeparator());
        }
        return ret;
    }
    ...
    private void handleBefore(ProceedingJoinPoint joinPoint) {
        /*
            如果看到了XXXHolder，一般是使用ThreadLocal进行数据共享。保证多个线程之间资源的隔离。
            父类(接口)不能调用子类的独有方法，需要向下转型。
         */
